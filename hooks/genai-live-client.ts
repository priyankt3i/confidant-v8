...
// FIX: Refactor to use composition for EventEmitter.
export class GenAILiveClient {
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      // FIX: Use this.emitter.emit
      this.emitter.emit('error', new ErrorEvent('Client is not connected'));
      return;
    }
    const partsArray = Array.isArray(parts) ? parts : [parts];
    // FIX: Corrected the `turns` object to match the `Content` type from `@google/genai`, which requires a `role` property.
    const turns = [{ role: 'user', parts: partsArray }];
